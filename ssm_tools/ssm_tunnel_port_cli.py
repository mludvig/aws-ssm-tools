...
def parse_args():
    ...
    group_network.add_argument('--port', metavar="PORT", dest="port", type=str,
        help='Port to route through ssm tunnel.')
    group_network.add_argument('--az', metavar="AZ", dest="az", type=str,
        help='az of ssm instance to tunnel through')
    group_network.add_argument('--os-user', metavar="OSUSER", dest="osuser", type=str,
        help='OS user to tunnel with')
    group_network.add_argument('--endpoint', metavar="ENDPOINT", dest="endpoint", type=str,
...
def gen_key():
    subprocess.call('ssh-keygen -t rsa -f /tmp/temp -N "" 2>/dev/null <<< y >/dev/null', shell=True)
...
def start_portfwd_session(instance_id, port, endpoint, os_user, profile=None, region=None):
    extra_args = ""
    if profile:
        extra_args += f"--profile {profile} "
    if region:
...
def main():
    ...
    try:
        ...
        if not instance_id:
            ...
            quit(1)
        send_ssh_key(instance_id, os_user=args.osuser, az=args.az, profile=args.profile, region=args.region)
        start_portfwd_session(instance_id, port=args.port, endpoint=args.endpoint, os_user=args.osuser, profile=args.profile, region=args.region)
